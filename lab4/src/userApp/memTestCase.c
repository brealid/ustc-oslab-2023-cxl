...
void memTestCaseInit(void){	
	registerCommand("testMalloc1\0", "Malloc, write and read.\0", testCase1);
	registerCommand("testMalloc2\0", "Malloc, write and read.\0", testCase2);		
	registerCommand("maxMallocSizeNow\0", "MAX_MALLOC_SIZE always changes. What's the value Now?\0", maxMallocSizeNow);

	registerCommand("testdP1\0", "Init a dPatition(size=0x100) [Alloc,Free]* with step = 0x20\0", testdP1);
	registerCommand("testdP2\0", "Init a dPatition(size=0x100) A:B:C:- ==> -:B:C:- ==> -:C:- ==> - .\0", testdP2);
	registerCommand("testdP3\0", "Init a dPatition(size=0x100) A:B:C:- ==> A:B:- ==> A:- ==> - .\0", testdP3);
	
	registerCommand("testeFP\0", "Init a eFPatition. Alloc all and Free all.\0", testeFP);
}
